...
const Blogs = ({ posts }) => {
    return (
        <main className={`${sass.Main}`}>
            <h2 className={`${sass.Heading} text-4xl my-10`} posts-aos="fade-up">Recently Posts</h2>
            <div className={`${sass.RecentPosts} flex flex-col items-center mb-8 `}>
                <div className={`${sass.col} grid sm:grid-cols-2 md:grid-cols-4 gap-5 mb-10 w-full`}>
                    {
                        posts.reverse().filter(post => post.id < 5).map(({ id, title, content, date }) => (
                            <Link href={`posts/${id}`} key={id} className={`${sass.post} p-3 w-full`} posts-aos="fade-up">
                                ...
                                <p className={`${sass.postBody} text-lg break-words`}>{content.substr(0, 150)}...</p>
                            </Link>
                        ))
                    }
                </div>
                <Link href="/posts" className={`${sass.Btn}`} posts-aos="fade-up" posts-aos-duration="700">See More</Link>
            </div>
        </main>
    )
}
